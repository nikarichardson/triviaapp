...
class QuestionView extends Component {
  ...
  getQuestions = () => {
    $.ajax({ 
      ...
      error: (error) => {
        alert('Unable to load questions. Please try your request again here')
        return;
      }
    })
  }
    ...
    $.ajax({
      ...
      success: (result) => {
        this.setState({
          ...
          totalQuestions: result.totalQuestions,
          categories: result.categories,
          currentCategory: result.categories })
        return;
      },
      error: (error) => {
  ...
  submitSearch = (searchTerm) => {
    $.ajax({
      ...
      error: (error) => {
        alert('Unable to load questions from search. Please try your request again')
        return;
      }
    })
  }
    ...
    if(action === 'DELETE') {
      if(window.confirm('Are you sure you want to delete the question?')) {
        $.ajax({
          ...
          type: "DELETE",
          success: (result) => {
            alert('You successfully deleted a question.')
            this.getQuestions();
          },
          error: (error) => {
